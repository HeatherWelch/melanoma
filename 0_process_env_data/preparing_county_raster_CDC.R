...
a=a%>% mutate(countyFIPS=str_pad(countyFIPS,5,side="left",pad=0)) %>% mutate(COUNTY_FIPS=as.character(countyFIPS))%>% group_by(COUNTY_FIPS) %>% summarize(Value=mean(as.numeric(Value)))
...
st_write(b, glue("{spatial_dir}/{variable}.shp"))
