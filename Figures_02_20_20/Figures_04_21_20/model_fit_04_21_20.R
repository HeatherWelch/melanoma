...
a=modDF %>% dplyr::select(-c(X,COUNTY_FIPS,STATEFP,NAME)) %>% mutate(Pop=as.numeric(gsub(",","",Pop))) %>% .[complete.cases(.),]
M <- cor(a)
...
png(glue("{outdir}/corplot_unweighted_POP.png"),width=15,height=15,type = "cairo",units='cm',res=400)
par(ps=10)
par(mar=c(4,4,1,1))
par(cex=1)
corrplot(M,type="upper",order="hclust",outline = T,tl.col="black",tl.cex = .9)
...
# seer rate all data ####
modDF=read.csv("/Users/heatherwelch/Dropbox/melenoma/Figures_04_20_20/master_dataframe.csv") %>% dplyr::select(-c(X,COUNTY_FIPS,STATEFP,NAME))
empty=list()
...
for(i in 2:ncol(modDF)){
  ...
  fm4 <- lmer(as.formula(glue("SEER_rate ~ {var}+(1|Pop)")), data = modDF)
...
# master=do.call(rbind,empty) %>% filter(variable!="STATEFP" &  variable!="NAME"&  variable!="Pop")
master=do.call(rbind,empty) %>% filter( variable=="Pop")
lmixedENVT=lmer(SEER_rate ~ UV_irradiance+cancer_gov_UV_exposure+elevation+seasonality_cloud+seasonality_temperature+(1|Pop),data=modDF)
lmixedmed=lmer(SEER_rate ~ incm_pc+incm_mh+wpovr50+derm_pk+pcp_pk+(1|Pop),data=modDF)
