...
mappfunction=function(dat,mapVar,varName){
# dat=dat[1:10,]
  ...
  map=ggplot(data=dat)+geom_sf(aes(fill=ntile(.data[[mapVar]],100)), size = 0.4,color="black")+ theme(panel.background = element_blank())+ theme(axis.line = element_line(colour = "black"))+
    scale_fill_gradientn(varName,breaks=rescale(labels,c(0,100)),labels=labels,colours = pals::parula(100),na.value="black")+
  ...
  # if(length(pretty(datt$new,n=4))==5){
  # map=map+scale_fill_gradientn(varName,breaks=rescale(labels,c(0,100)),labels=pretty(datt$new,n=4),colours = pals::parula(100),na.value="black")
  # } else if (length(pretty(datt$new,n=4))==4){
  #   map=map+scale_fill_gradientn(varName,breaks=rescale(labels,c(0,100)),labels=pretty(datt$new,n=4),colours = pals::parula(100),na.value="black")
  # } else if (length(pretty(datt$new,n=4))==6){
  #   map=map+scale_fill_gradientn(varName,breaks=rescale(labels,c(0,100)),labels=pretty(datt$new,n=4),colours = pals::parula(100),na.value="black")
  # }
...
predictions=names(dat)[c(20,33:40)]
...
system.time(print(
  foreach(i=1:length(predictions),.export = c("mappfunction","outdir","predictions","dat"),.packages = c("ggplot2","glue","maps","tidyverse","scales"),.verbose=T) %dopar% {
   # old junk with lablels ####
    ...
    mappfunction(mapVar=predictions[i],dat=dat,varName = varName)
  }
))
...
# # 
# datt=dat[1:10,]
# variable=rlang::sym(mapVar)
# dattt=dat %>% mutate(mapfield=ntile(.data[[mapVar]],100)) %>% group_by(mapfield) %>% summarise(mean=mean(.data[[mapVar]]))
# 
# map.world = map_data(map="usa")
# map=ggplot()+geom_sf(data=dattt,aes(fill = mapfield), size = 0.4)+ theme(panel.background = element_blank())+ theme(axis.line = element_line(colour = "black"))+
#   scale_fill_gradientn(mapVar,colours = pals::parula(100),na.value="black")+
#   #scale_color_gradientn("Test",breaks=pretty(datt$AWATER,n=7),colours = pals::parula(100),na.value="black")+
#   coord_sf(xlim = c(-125.0011, -66.9326),ylim = c(24.9493,49.5904))+
#   theme(axis.line=element_blank(),
#         axis.text.x=element_blank(),
#         axis.text.y=element_blank(),
...
#         panel.grid.minor=element_blank(),
#         plot.background=element_blank())+
#   theme(legend.title = element_text(size=8),legend.position=c(.9,.4),legend.key.width = unit(1.5, "cm"),legend.key.height = unit(1.4, "cm"))+theme(legend.text=element_text(size=8),legend.title = element_text(size=10))
# 
# map

# png(glue("/Users/heatherwelch/Dropbox/melenoma/Figures_04_20_20/test.png"),width=36,height=22,type = "cairo",units='cm',res=400)
# par(ps=10)
# par(mar=c(4,4,1,1))
# par(cex=1)
# print({map})
