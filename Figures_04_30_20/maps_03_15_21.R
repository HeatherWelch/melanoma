...
mappfunction=function(dat,mapVar){
  ...
  if(varName=="Health insurance"||varName=="% non-Hispanic white households with income > $100,000"||
    ...
    map=ggplot()+geom_sf(data=dat, size = 0.4,color="#F5F5F5",fill="#A9A9A9")+
      scale_fill_manual("",values=c("#A9A9A9"),labels=c("No data"))+
    ...
    map=map+new_scale_fill()+geom_sf(data=dat,aes(fill=ntile(.data[[mapVar]],100)), size = 0.4,color="black")+ 
      scale_fill_gradientn("",breaks=rescale(labels,c(0,100)),labels=labels2,colours = pals::parula(100),na.value=NA)+
      coord_sf(xlim = c(-125.0011, -66.9326),ylim = c(24.9493,49.5904))
  ...
  png(glue("{outdir}/{mapVar}2.png"),width=36,height=22,type = "cairo",units='cm',res=400)
  par(ps=10)
  par(mar=c(4,4,1,1))
  par(cex=1)
  print({map})
