...
# dat=dat[1:10,]
  ...
  map=ggplot(data=dat)+geom_sf(aes(fill=ntile(.data[[mapVar]],100)), size = 0.4,color="black")+ theme(panel.background = element_blank())+ theme(axis.line = element_line(colour = "black"))+
    ...
    theme(legend.title = element_text(size=8),legend.position=c(.92,.4),legend.key.width = unit(1.5, "cm"),legend.key.height = unit(1.4, "cm"))+theme(legend.text=element_text(size=9),legend.title = element_text(size=9))+
    ggtitle(varName)
  } else {
