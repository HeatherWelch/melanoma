...
ui <- dashboardPage(skin="black",
                    ...
                    dashboardSidebar(
                      ...
                      sidebarMenu(id = 'sidebarmenu',
                                  ...
                                  varSelectInput("ME", "Medical care exposure variables", masterME, selected = "Primary care", multiple = T,
                                                 selectize = T, width = NULL, size = NULL),
                                  sliderInput("slider1", label = "Control for county poulation", min = 1978, 
                                              max = 14040440, value = c(1978,10000000))
                                  # div(style="text-align:center",actionButton("GO", "Build model"))
                    ...
                    dashboardBody(
                      ...
                      absolutePanel(verbatimTextOutput("placeholderPop"), top=180, right=100))
                    ))
...
server <- shinyServer(function(input, output) {
  output$hcontainer <- renderPlot({
    UVDF=masterUV %>% dplyr::select(!!!input$UV)%>% mutate(Pop=master$Pop)%>% mutate(SEER_rate=master$SEER_rate,In.situ=master$In.situ,Invasive=master$Invasive) %>% filter(Pop>input$slider1[1]&Pop<input$slider1[2])
    MEDF=masterME %>% dplyr::select(!!!input$ME)%>% mutate(Pop=master$Pop)%>% mutate(SEER_rate=master$SEER_rate,In.situ=master$In.situ,Invasive=master$Invasive) %>% filter(Pop>input$slider1[1]&Pop<input$slider1[2])
  ...
  output$placeholderPop=renderText({
    MEDF=masterME%>% mutate(Pop=master$Pop) %>% filter(Pop>input$slider1[1]&Pop<input$slider1[2])
    ncounties=nrow(MEDF)
    low=input$slider1[1] %>% as.numeric() %>% comma_format()
     form1a=glue("Counties with populations between {input$slider1[1]}  and {input$slider1[2]} (n={ncounties})}")
    # form1a=glue("Counties with populations between {low} (n={ncounties})}")
     form1a
    # low
    # ncounties
